...
```{r}
# Load the dataset
studentData <- read.csv("~/Downloads/BTRY 6020/Final Project/Data/student_habits_performance.csv")
names(studentData)
dim(studentData)
```
...
## Validating mMdel using Appropriate Cross-Validation Techniques
...
The initial model that was selected using the branch and bound method and assessed using BIC, yielded the best predictive performance. However, this model violated the assumption of normality of residuals. Attempts to transform the dependent variable did not resolve this. Instead, transforming selected covariates (log(mental_health_rating)) improved the residual normality, satisfying this linear model assumption. However, this came at the cost of a significantly worse BIC (~115 points higher) and slightly worse cross-validation error (untransformed model RSME = 5.35, transformed model RSME = 5.69).
